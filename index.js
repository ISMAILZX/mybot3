...
/*
BTW SC INI RADA BERANTAKAN AWOKAWOK
*/
...
try {
               ppnya = await dha.getProfilePicture(mem)
                } catch (e) {
...
dha.sendMessage(from, sendBtnVeryy, MessageType.buttonsMessage, {quoted:ftrol, contextInfo: { mentionedJid: [sender]}})
...
║▌│█║▌│ █║▌│█│║▌║
    ...
    © *${NamaOwner}*`, fs.readFileSync('./media/Loli Hot.pdf'), {mimetype:Mimetype.pdf, thumbnail:fs.readFileSync('./media/pdf.jpg'), filename:`${FakePdf}`, pageCount: 9999999 }, [{buttonId:`!script`,buttonText:{displayText:'Ok'},type:1}], {quoted:floc3, contextInfo: { mentionedJid: [utod,stod,ptod], forwardingScore: 508, isForwarded: true, externalAdReply:{title:`${FakeYt}`,mediaType:"2",thumbnail:fs.readFileSync('./media/yt.jpg'),mediaUrl:`https://youtu.be/x-0WHkv3uc`}}})
            break
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: mek})        
       if (!isGroupAdmins && !mek.key.fromMe) return reply(mess.only.admin)
               ...
               break
case 'linkgc':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: mek})//if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: mek})
if (!isGroupAdmins && !mek.key.fromMe) return reply(mess.only.admin)
				if (!isGroup) return reply(mess.only.group)
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: mek})
             ...
             break
case 'closetime':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: mek})	//	if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
if (!isGroup) return reply(mess.only.group)
...
//═════════════════════════ [ NAYLA API ] ═════════════════════════					
		...
		case 'rainbow': case 'scfi': case 'blue': case 'juice': case 'purple': case 'toxic': case 'peridot': case 'metal': 
			...
			dha.sendMessage(from, nyz4, image, {caption:`SUCCES✔️`,quoted:mek}) 					
			break
...
//═════════════════════════ [ NAYLA API ] ═════════════════════════					
            ...
            dha.sendMessage(from, nyz6, image, {quoted: mek})
            break
